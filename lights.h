...
int calcLuxAverage() {
	...
	luxmed = ((luxmed / LIGHT_LINE_NUMBER) * 100) / 255;
	return int(luxmed);
}
...
void LightsHandler() {
	...
	uint8_t pwmValue = Plafo[channel].pwmValue;				// save the old value

	if(Plafo[channel].workingMode == POWER_OFF) {
		if(pwmValue <= 0)	{				// Se sono arrivato al fading minimo e sono in OFF manuale disattivo lo switch
			Plafo[channel].powerState = POWER_OFF;
		} else {	
			if(((myMillis() - Plafo[channel].fadingTimer) >= FADING_INTERVAL) && (pwmValue > 0)) {	
				pwmValue--;
				Plafo[channel].fadingTimer = myMillis();	//Plafo[channel].fadingTimer + FADING_INTERVAL;
			}
		}
	} else if(Plafo[channel].workingMode == POWER_ON) {
		if(pwmValue >= Plafo[channel].pwmMax)	{	// Se sono arrivato al fading massimo e sono in ON manuale disattivo lo switch
			Plafo[channel].powerState = POWER_ON;
			if(((myMillis() - Plafo[channel].fadingTimer) >= FADING_INTERVAL) && (pwmValue < Plafo[channel].pwmMax)) {	
				pwmValue++;
				Plafo[channel].fadingTimer = myMillis();	//Plafo[channel].fadingTimer + FADING_INTERVAL;
			}
		}
...
/*		if(Plafo[channel].powerState > POWER_OFF) {
			if(minsFA < minsOn) minsFA += NINUTES_PER_DAY;
			if(minsIT < minsOn) minsIT += NINUTES_PER_DAY;
			if(minsOff < minsOn) minsOn += NINUTES_PER_DAY;
//			if(curTime < minsOn) minsOn += NINUTES_PER_DAY;
		}
*/
		if(minsIT > minsFA) {
			...
			if(curTime > minsOff and curTime < minsOn) {									// fase di buio
				if(Plafo[channel].powerState == POWER_ON_DEC or Plafo[channel].startup) {
					Plafo[channel].powerState = POWER_OFF;
					pwmValue = 0;
					DEBUG(F("Light %d is now off\n"), channel+1);
			...
			} else if(curTime >= minsFA and curTime <= NINUTES_PER_DAY or curTime >= 0 and curTime <= minsIT) {		// fase di luce piena
				if(Plafo[channel].powerState == POWER_ON_INC or Plafo[channel].startup) {
					Plafo[channel].powerState = POWER_ON;
					pwmValue = pwmMax;
					DEBUG(F("Light %d is now on\n"), channel+1);
