...
void WaterTurbidityHandler() {
	...
	if((ntuTimer + NTU_READ_INTERVAL) < millis()) {					//time interval: 1s
		...
		if( abs(int(NTUavg - ntu)) > 1 ) {
			...
			Serial.print(F("Water turbidity is "));
			Serial.print(NTUavg);
			Serial.print(F(" ntu"));
			Serial.println();
#endif		
		...
		if(main_page == 1 and dstatus == DS_IDLE) {
			printSpaces(7, 13, 3);
			printString(isnan(NTUavg) ? NOT_AVAILABLE : ftoa(buff, ntu), 13, 3);
#if defined SR_TURBIDITY
			printChar(relaisStatus(SR_TURBIDITY)?'*':' ', 0, 3);
#endif
		}
